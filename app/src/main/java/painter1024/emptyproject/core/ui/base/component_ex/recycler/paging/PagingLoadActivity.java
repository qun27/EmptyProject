...
public abstract class PagingLoadActivity extends BaseActivity
    ...
    protected SwipeRefreshLayout prepareSwipeRefreshLayout(){
        View view = findViewById(R.id.refreshLayout);
    ...
    protected void onInitRefreshLayout(SwipeRefreshLayout refreshLayout) {
        refreshLayout.setColorSchemeColors(ResUtil.getColorOfAttr(this, R.attr.colorAccent));
        //设置load的偏移值，避免5.0以下系统，下拉上滑后，load还在。
        //其中false代表不缩放，-100是起始位置，结束位置和源码的一致。
        refreshLayout.setProgressViewOffset(false, -100, (int) (64 * (getResources().getDisplayMetrics().density)));
    }
...
//        if(recyclerView != null) return recyclerView.getAdapter();
    ...
    private RecyclerView.Adapter ensureLastPageFooterAdapter(RecyclerView.Adapter adapter){
        if (adapter instanceof PagingFooterAdapter) {
            return pagingFooterAdapter = (PagingFooterAdapter) adapter;
        } else {
            return pagingFooterAdapter = new PagingFooterAdapter(adapter);
        }
    }
    ...
    /**
     * 获取RecyclerView<br/>
     * 在{@link #onPostCreate(Bundle)}之后才能被调用
     */
